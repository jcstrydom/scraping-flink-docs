> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](https://nightlies.apache.org/flink/flink-docs-stable/docs/deployment/memory/network_mem_tuning/).

# Network memory tuning guide  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#network-memory-tuning-guide)

## Overview  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#overview)

Each record in Flink is sent to the next subtask compounded with other records in a _network buffer_,
the smallest unit for communication between subtasks. In order to maintain consistent high throughput,
Flink uses _network buffer queues_ (also known as _in-flight data_) on the input and output side of the transmission process.
...
barriers. In [aligned checkpoints](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/concepts/stateful-stream-processing/#checkpointing), those checkpoint barriers are traveling throughout the job graph along with
the network buffers. The larger the amount of in-flight data, the longer the checkpoint barrier propagation time. In [unaligned checkpoints](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/concepts/stateful-stream-processing/#unaligned-checkpointing), the larger the amount of in-flight data, the larger the checkpoint size will be because all of the captured in-flight data has to be persisted as part of the checkpoint.

## The Buffer Debloating Mechanism  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#the-buffer-debloating-mechanism)
...
Consult the [configuration](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#full-taskmanageroptions) documentation for more details and additional parameters.

Here are [metrics](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/metrics/#io) you can use to monitor the current buffer size:
...
### Limitations  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#limitations)

Currently, there are a few cases that are not handled automatically by the buffer debloating mechanism.

#### Multiple inputs and unions  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#multiple-inputs-and-unions)
...
#### Buffer size and number of buffers  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#buffer-size-and-number-of-buffers)
...
#### High parallelism  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#high-parallelism)
...
## Network buffer lifecycle  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#network-buffer-lifecycle)
...
### Input network buffers  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#input-network-buffers)
...
### Output network buffers  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#output-network-buffers)
...
#### Overdraft buffers  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#overdraft-buffers)
...
## The number of in-flight buffers  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#the-number-of-in-flight-buffers)
...
### Selecting the buffer size  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#selecting-the-buffer-size)
...
### Selecting the buffer count  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#selecting-the-buffer-count)
...
and floating buffers(except you have one of [limit cases](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/#limitations)). If the amount of in-flight data is causing issues, enabling
[buffer debloating](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/#the-buffer-debloating-mechanism) is recommended.
...
Using the buffer roundtrip time (around `1ms` in a healthy local network), [the buffer size](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#taskmanager-memory-segment-size), and the expected throughput, you can calculate the number of buffers required to sustain the throughput by using this formula:
...
In the case of backpressure in low throughput setups, you should consider reducing the number of [exclusive buffers](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#taskmanager-network-memory-buffers-per-channel).

## Summary  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/network_mem_tuning/\#summary)

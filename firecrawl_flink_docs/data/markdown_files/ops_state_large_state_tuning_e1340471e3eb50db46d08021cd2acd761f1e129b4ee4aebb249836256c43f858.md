# Tuning Checkpoints and Large State  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#tuning-checkpoints-and-large-state)

This page gives a guide how to configure and tune applications that use large state.

## Overview  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#overview)
...
## Monitoring State and Checkpoints  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#monitoring-state-and-checkpoints)

The easiest way to monitor checkpoint behavior is via the UI’s checkpoint section. The documentation
for [checkpoint monitoring](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/monitoring/checkpoint_monitoring/) shows how to access the available checkpoint
metrics.

The two numbers (both exposed via Task level [metrics](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#checkpointing)
and in the [web interface](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/monitoring/checkpoint_monitoring/)) that are of particular interest when scaling
up checkpoints are:

- The time until operators receive their first checkpoint barrier
When the time to trigger the checkpoint is constantly very high, it means that the _checkpoint barriers_ need a long
...
[Unaligned checkpoints](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpoints/#unaligned-checkpoints) can be used to speed up the propagation time
...
![Illustration how the minimum-time-between-checkpoints parameter affects checkpointing behavior.](https://nightlies.apache.org/flink/flink-docs-release-2.2/fig/checkpoint_tuning.svg)

_Note:_ Applications can be configured (via the `CheckpointConfig`) to allow multiple checkpoints to be in progress at
...
## Tuning RocksDB or ForSt  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#tuning-rocksdb-or-forst)

The state storage workhorse of many large scale Flink streaming applications is the _RocksDB State Backend_.
The backend scales well beyond main memory and reliably stores large [keyed state](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/datastream/fault-tolerance/state/).

If you are handling very large state, even exceeding the local disk space of the TaskManagers, you may want to consider
using the disaggregated state store [ForStStateBackend](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#forst-state-backend).
This backend stores the state in a separate storage system, such as HDFS or S3, and only keeps the
state metadata and cache in the TaskManagers. And the [State API V2](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/datastream/fault-tolerance/state_v2/)
...
### Incremental Checkpoints  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#incremental-checkpoints)
...
See [Incremental Checkpoints in RocksDB](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#incremental-checkpoints) for more background information.

### Timers in RocksDB or on JVM Heap  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#timers-in-rocksdb-or-on-jvm-heap)
...
See [this section](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#timers-heap-vs-rocksdb) for details on how to configure heap-based timers.

### Tuning RocksDB Memory  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#tuning-rocksdb-memory)
...
By default, the RocksDB State Backend uses Flink’s managed memory budget for RocksDBs buffers and caches (`state.backend.rocksdb.memory.managed: true`). Please refer to the [RocksDB Memory Management](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#memory-management) for background on how that mechanism works.
...
## Capacity Planning  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#capacity-planning)
...
- Normal operation should have enough capacity to not operate under constant _back pressure_.
See [back pressure monitoring](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/monitoring/back_pressure/) for details on how to check whether the application runs under back pressure.
...
_Important_: The base line should to be established with checkpointing activated, because checkpointing ties up
...
**Important:** In order to allow for adding resources later, make sure to set the _maximum parallelism_ of the
...
Flink’s internal bookkeeping tracks parallel state in the granularity of max-parallelism-many _key groups_.
...
## Compression  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#compression)
...
## Task-Local Recovery  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#task-local-recovery)

### Motivation  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#motivation)
...
failures), but we still have to read remote state. This can result in _long recovery time for large states_, even if there was only a small failure on
a single machine.

### Approach  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#approach)
...
does not only write task states to the distributed storage, but also keep _a secondary copy of the state snapshot in a storage that is local to_
_the task_ (e.g. on local disk or in memory). Notice that the primary store for snapshots must still be the distributed store, because local storage
...
copy and avoid the costs of reading the state remotely. Given that _many failures are not node failures and node failures typically only affect one_
_or very few nodes at a time_, it is very likely that in a recovery most tasks can return to their previous location and find their local state intact.
...
![Illustration of checkpointing with task-local recovery.](https://nightlies.apache.org/flink/flink-docs-release-2.2/fig/local_recovery.png)

### Relationship of primary (distributed store) and secondary (task-local) state snapshots  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#relationship-of-primary-distributed-store-and-secondary-task-local-state-snapshots)
...
- For checkpointing, the _primary copy must be successful_ and a failure to produce the _secondary, local copy will not fail_ the checkpoint. A checkpoint
...
- For recovery, Flink will always _attempt to restore from task-local state first_, if a matching secondary copy is available. If any problem occurs during
the recovery from the secondary copy, Flink will _transparently retry to recover the task from the primary copy_. Recovery only fails, if primary
...
a _superset of the task-local state_.
...
### Configuring task-local recovery  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#configuring-task-local-recovery)

Task-local recovery is _deactivated by default_ and can be activated through Flink’s configuration with the key `state.backend.local-recovery` as specified
in `CheckpointingOptions.LOCAL_RECOVERY`. The value for this setting can either be _true_ to enable or _false_ (default) to disable local recovery.

Note that [unaligned checkpoints](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpoints/#unaligned-checkpoints) currently do not support task-local recovery.

### Details on task-local recovery for different state backends  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#details-on-task-local-recovery-for-different-state-backends)

_**Limitation**: Currently, task-local recovery only covers keyed state backends. Keyed state is typically by far the largest part of the state. In the near future, we will_
_also cover operator state and timers._
...
- **EmbeddedRocksDBStateBackend**: task-local recovery is supported for keyed state. For _full checkpoints_, state is duplicated to a local file. This can introduce additional write costs
and occupy local disk space. For _incremental snapshots_, the local state is based on RocksDB’s native checkpointing mechanism. This mechanism is also used as the first step
...
### Allocation-preserving scheduling  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/\#allocation-preserving-scheduling)
...
allocation and _requests the exact same slot_ to restart in recovery. If this slot is not available, the task will request a _new, fresh slot_ from the resource manager. This way,
if a task manager is no longer available, a task that cannot return to its previous location _will not drive other recovering tasks out of their previous slots_. Our reasoning is
that the previous slot can only disappear when a task manager is no longer available, and in this case _some_ tasks have to request a new slot anyways. With our scheduling strategy

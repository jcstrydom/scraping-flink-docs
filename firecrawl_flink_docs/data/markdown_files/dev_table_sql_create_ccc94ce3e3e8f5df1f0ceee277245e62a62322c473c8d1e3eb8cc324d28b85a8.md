# CREATE Statements  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-statements)

CREATE statements are used to register a table/view/function into current or specified [Catalog](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/catalogs/). A registered table/view/function can be used in SQL queries.
...
## Run a CREATE statement  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#run-a-create-statement)
...
CREATE statements can be executed in [SQL CLI](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sqlclient/).
...
## CREATE TABLE  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-table)
...
### Columns  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#columns)
...
operations. The [connector and format documentation](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/overview/) lists the
...
or [watermark strategy push down](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sourcessinks/), the evaluation might be spread
...
Computed columns are commonly used in Flink for defining [time attributes](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/time_attributes/)
in `CREATE TABLE` statements.

- A [processing time attribute](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/time_attributes/#processing-time)
can be defined easily via `proc AS PROCTIME()` using the system’s `PROCTIME()` function.
- An [event time attribute](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/time_attributes/#event-time) timestamp
...
### `WATERMARK` [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#watermark)
...
Watermark is emitted in an interval defined by [`pipeline.auto-watermark-interval`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#pipeline-auto-watermark-interval) configuration.
...
### `PRIMARY KEY` [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#primary-key)
...
### `PARTITIONED BY` [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#partitioned-by)
...
### `DISTRIBUTED` [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#distributed)
...
### `WITH` Options  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#with-options)
...
The key and value of expression `key1=val1` should both be string literal. See details in [Connect to External Systems](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/overview/) for all the supported table properties of different connectors.

**Notes:** The table name can be of three formats: 1. `catalog_name.db_name.table_name` 2\. `db_name.table_name` 3\. `table_name`. For `catalog_name.db_name.table_name`, the table would be registered into metastore with catalog named “catalog\_name” and database named “db\_name”; for `db_name.table_name`, the table would be registered into the current catalog of the execution table environment and database named “db\_name”; for `table_name`, the table would be registered into the current catalog and database of the execution table environment.
...
### `LIKE` [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#like)
...
### `AS select_statement` [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#as-select_statement)
...
There are two parts in CTAS, the SELECT part can be any [SELECT query](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/queries/overview/) supported by Flink SQL. The CREATE part takes the resulting schema from the SELECT part and creates the target table. Similar to `CREATE TABLE`, CTAS requires the required options of the target table must be specified in WITH clause.
...
#### Atomicity  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#atomicity)

If you want to enable atomicity for CTAS, then you should make sure:

- The sink has implemented the atomicity semantics for CTAS. You may refer to the doc for the corresponding connector sink to know the atomicity semantics is available or not. For devs who want to implement the atomicity semantics, please refer to the doc [SupportsStaging](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sourcessinks/#sink-abilities).
- Set option [table.rtas-ctas.atomicity-enabled](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/config/#table-rtas-ctas-atomicity-enabled) to `true`.

## \[CREATE OR\] REPLACE TABLE  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-or-replace-table)
...
There are two parts in RTAS: the SELECT part can be any [SELECT query](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/queries/overview/) supported by Flink SQL, the `REPLACE TABLE` part takes the resulting schema from the `SELECT` part and replace the target table. Similar to `CREATE TABLE` and `CTAS`, RTAS requires the required options of the target table must be specified in WITH clause.
...
### Atomicity  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#atomicity-1)
...
## CREATE CATALOG  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-catalog)
...
Check out more details at [Catalogs](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/catalogs/).

## CREATE DATABASE  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-database)
...
## CREATE VIEW  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-view)
...
## CREATE FUNCTION  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-function)
...
If the language tag is JAVA/SCALA, the identifier is the full classpath of the UDF. For the implementation of Java/Scala UDF, please refer to [User-defined Functions](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/functions/udfs/) for more details.

If the language tag is PYTHON, the identifier is the fully qualified name of the UDF, e.g. `pyflink.table.tests.test_udf.add`. For the implementation of Python UDF, please refer to [Python UDFs](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/python/table/udfs/python_udfs/) for more details.

If the language tag is PYTHON, however the current program is written in Java/Scala or pure SQL, then you need to [configure the Python dependencies](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/python/dependency_management/#python-dependency-in-javascala-program).
...
Specifies the list of jar resources that contain the implementation of the function along with its dependencies. The jar should be located in a local or remote [file system](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/filesystems/overview/) such as hdfs/s3/oss which Flink current supports.

Attention Currently only JAVA, SCALA language support USING clause.

## CREATE MODEL  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#create-model)
...
### Examples  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/create/\#examples)

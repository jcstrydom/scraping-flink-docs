# State Backends  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#state-backends)

Programs written in the [Data Stream API](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/datastream/overview/) often hold state in various forms:
...
See also [state section](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/datastream/fault-tolerance/state/) in the streaming API guide.
...
## Available State Backends  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#available-state-backends)
...
### The HashMapStateBackend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#the-hashmapstatebackend)

The _HashMapStateBackend_ holds data internally as objects on the Java heap. Key/value state and window operators hold hash tables
...
It is also recommended to set [managed memory](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/memory/mem_setup_tm/#managed-memory) to zero.
...
### The EmbeddedRocksDBStateBackend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#the-embeddedrocksdbstatebackend)
...
Check also recommendations about the [task executor memory configuration](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/memory/mem_tuning/#rocksdb-state-backend) for the EmbeddedRocksDBStateBackend.

EmbeddedRocksDBStateBackend is currently the only backend that offers incremental checkpoints (see [here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/)).

Certain RocksDB native metrics are available but disabled by default, you can find full documentation [here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#rocksdb-native-metrics)

The total memory amount of RocksDB instance(s) per slot can also be bounded, please refer to documentation [here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/#bounding-rocksdb-memory-usage) for details.

## The ForStStateBackend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#the-forststatebackend)

The _ForStStateBackend_ is a state backend that is based on [ForSt project](https://github.com/ververica/ForSt),
which is also a LSM-tree structured key-value store and built on top of the RocksDB.
It is designed for disaggregated state management, for more details, see [here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/disaggregated_state/).
...
State API V2, please refer to the [State API V2 documentation](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/datastream/fault-tolerance/state_v2/).

## Choose The Right State Backend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#choose-the-right-state-backend)
...
## Configuring a State Backend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#configuring-a-state-backend)

The default state backend, if you specify nothing, is the `HashMapStateBackend`. If you wish to establish a different default for all jobs on your cluster, you can do so by defining a new default state backend in [**Flink configuration file**](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file). The default state backend can be overridden on a per-job basis, as shown below.

### Setting the Per-job State Backend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#setting-the-per-job-state-backend)
...
> Since RocksDB and ForSt is part of the default Flink distribution, you do not need this dependency if you are not using any RocksDB code in your job and configure the state backend via `state.backend.type` and further [checkpointing](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#checkpointing) and [RocksDB-specific](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#rocksdb-state-backend) or [ForSt-specific](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#forst-state-backend) parameters in your [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file).

### Setting Default State Backend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#setting-default-state-backend)

A default state backend can be configured in the [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file), using the configuration key `state.backend.type`.

Possible values for the config entry are _hashmap_ (HashMapStateBackend), _rocksdb_ (EmbeddedRocksDBStateBackend), _forst_ (ForStStateBackend) or the fully qualified class
name of the class that implements the state backend factory
[StateBackendFactory](https://github.com/apache/flink/blob/release-2.2/flink-runtime/src/main/java/org/apache/flink/runtime/state/StateBackendFactory.java)
,
...
You can find more details about the checkpoint directory structure [here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpoints/#directory-structure).
...
## RocksDB State Backend Details  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#rocksdb-state-backend-details)

_This section describes the RocksDB state backend in more detail._

### Incremental Checkpoints  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#incremental-checkpoints)

RocksDB supports _Incremental Checkpoints_, which can dramatically reduce the checkpointing time in comparison to full checkpoints.
...
- Setting a default in your [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file): `execution.checkpointing.incremental: true` will enable incremental checkpoints, unless the application overrides this setting in the code.
...
### Memory Management  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#memory-management)
...
Users can choose to deactivate that feature and let RocksDB allocate memory independently per ColumnFamily (one per state per operator). This offers expert users ultimately more fine grained control over RocksDB, but means that users need to take care themselves that the overall memory consumption does not exceed the limits of the environment. See [large state tuning](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/#tuning-rocksdb-memory) for some guideline about large state performance tuning.
...
> When the above described mechanism (`cache` and `write buffer manager`) is enabled, it will override any customized settings for block caches and write buffers done via [`PredefinedOptions`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#predefined-per-columnfamily-options) and [`RocksDBOptionsFactory`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#passing-options-factory-to-rocksdb).

Expert Mode

To control memory manually, you can set `state.backend.rocksdb.memory.managed` to `false` and configure RocksDB via [`ColumnFamilyOptions`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#passing-options-factory-to-rocksdb). Alternatively, you can use the above mentioned cache/buffer-manager mechanism, but set the memory size to a fixed amount independent of Flink’s managed memory size (`state.backend.rocksdb.memory.fixed-per-slot` or `state.backend.rocksdb.memory.fixed-per-tm` options). Note that in both cases, users need to ensure on their own that enough memory is available outside the JVM for RocksDB.


### Timers (Heap vs. RocksDB)  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#timers-heap-vs-rocksdb)
...
### Enabling RocksDB Native Metrics  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#enabling-rocksdb-native-metrics)

You can optionally access RockDB’s native metrics through Flink’s metrics system, by enabling certain metrics selectively.
See [configuration docs](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#rocksdb-native-metrics) for details.

> Enabling RocksDB’s native metrics may have a negative performance impact on your application.

### Advanced RocksDB Memory Turning  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#advanced-rocksdb-memory-turning)

> Flink offers sophisticated default [memory management for RocksDB](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#memory-management) that should work for most use-cases. The below mechanisms should mainly be used for _expert tuning_ or _trouble shooting_.

#### Predefined Per-ColumnFamily Options  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#predefined-per-columnfamily-options)
...
- Set the option’s name in [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file) via `state.backend.rocksdb.predefined-options`.
...
Predefined options set programmatically would override the ones configured via [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file).

#### Reading Column Family Options from Flink configuration file  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#reading-column-family-options-from-flink-configuration-file)

RocksDB State Backend picks up all config options [defined here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#advanced-rocksdb-state-backends-options). Hence, you can configure low-level Column Family options simply by turning off managed memory for RocksDB and putting the relevant entries in the configuration.

#### Passing Options Factory to RocksDB  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#passing-options-factory-to-rocksdb)
...
- Configure options factory class name in the [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file) via `state.backend.rocksdb.options-factory`.

- Set the options factory programmatically, e.g. `EmbeddedRocksDBStateBackend.setRocksDBOptions(new MyOptionsFactory());`


Options factory which set programmatically would override the one configured via [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file),
...
## Enabling Changelog  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#enabling-changelog)

### Introduction  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#introduction)
...
[Unaligned checkpoints](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpointing_under_backpressure/#unaligned-checkpoints)
and [Buffer debloating](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpointing_under_backpressure/#buffer-debloating)
2. Snapshot creation time (so-called synchronous phase), addressed by asynchronous snapshots (mentioned [above](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#the-embeddedrocksdbstatebackend))
3. Snapshot upload time (asynchronous phase)

Upload time can be decreased by [incremental checkpoints](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#incremental-checkpoints).
...
### Installation  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#installation)

Changelog JARs are included into the standard Flink distribution.

Make sure to [add](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/filesystems/overview/) the necessary filesystem plugins.

### Configuration  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#configuration)
...
Please keep the following defaults (see [limitations](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#limitations)):

```yaml
execution.checkpointing.max-concurrent-checkpoints: 1
```

Please refer to the [configuration section](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#state-changelog-options) for other options.
...
### Monitoring  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#monitoring)

Available metrics are listed [here](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#state-changelog).

If a task is backpressured by writing state changes, it will be shown as busy (red) in the UI.

### Upgrading existing jobs  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#upgrading-existing-jobs)
...
- take either a [savepoint](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/savepoints/#resuming-from-savepoints) or a [checkpoint](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpoints/#resuming-from-a-retained-checkpoint)
...
### Limitations  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#limitations)

- At most one concurrent checkpoint
- As of Flink 1.15, only `filesystem` changelog implementation is available
- [NO\_CLAIM](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#execution-savepoint-restore-mode) mode not supported

## Migrating from Legacy Backends  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#migrating-from-legacy-backends)
...
### MemoryStateBackend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#memorystatebackend)

The legacy `MemoryStateBackend` is equivalent to using [`HashMapStateBackend`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#the-hashmapstatebackend) and [`JobManagerCheckpointStorage`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpoints/#the-jobmanagercheckpointstorage).

#### Configuration using Flink configuration file  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#configuration-using-flink-configuration-file)
...
#### Code Configuration  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#code-configuration)
...
### FsStateBackend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#fsstatebackend)
...
#### Configuration using Flink configuration file  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#configuration-using-flink-configuration-file-1)
...
#### Code Configuration  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#code-configuration-1)
...
### RocksDBStateBackend  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#rocksdbstatebackend)

The legacy `RocksDBStateBackend` is equivalent to using [`EmbeddedRocksDBStateBackend`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#the-embeddedrocksdbstatebackend) and [`FileSystemCheckpointStorage`](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/checkpoints/#the-filesystemcheckpointstorage).

#### Configuration using Flink configuration file  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#configuration-using-flink-configuration-file-2)
...
#### Code Configuration  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/\#code-configuration-2)

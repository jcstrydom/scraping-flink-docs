> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](https://nightlies.apache.org/flink/flink-docs-stable/docs/deployment/resource-providers/yarn/).

# Apache Hadoop YARN  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#apache-hadoop-yarn)

## Getting Started  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#getting-started)

This _Getting Started_ section guides you through setting up a fully functional Flink Cluster on YARN.

### Introduction  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#introduction)
...
### Preparation  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#preparation)
...
### Starting a Flink Session on YARN  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#starting-a-flink-session-on-yarn)
...
## Deployment Modes Supported by Flink on YARN  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#deployment-modes-supported-by-flink-on-yarn)

For production use, we recommend deploying Flink Applications in [Application Mode](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/overview/#deployment-modes) as it provides a better isolation between applications.

### Application Mode  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#application-mode)
...
### Session Mode  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#session-mode)
...
We describe deployment with the Session Mode in the [Getting Started](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/#getting-started) guide at the top of the page.
...
Besides passing [configuration](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/) via the [Flink configuration file](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#flink-configuration-file), you can also pass any configuration at submission time to the `./bin/yarn-session.sh` client using `-Dkey=value` arguments.
...
### Per-Job Mode (deprecated)  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#per-job-mode-deprecated)
...
> For high-level intuition behind the per-job mode, please refer to the [deployment mode overview](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/overview/#per-job-mode).
...
## Flink on YARN Reference  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#flink-on-yarn-reference)

### Configuring Flink on YARN  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#configuring-flink-on-yarn)

The YARN-specific configurations are listed on the [configuration page](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#yarn).
...
### Resource Allocation Behavior  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#resource-allocation-behavior)
...
The memory configurations for JobManager and TaskManager processes will be respected by the YARN implementation. The number of reported VCores is by default equal to the number of configured slots per TaskManager. The [yarn.containers.vcores](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#yarn-containers-vcores) allows overwriting the number of vcores with a custom value. In order for this parameter to work you should enable CPU scheduling in your YARN cluster.

Failed containers (including the JobManager) are replaced by YARN. The maximum number of JobManager container restarts is configured via [yarn.application-attempts](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#yarn-application-attempts) (default 1). The YARN Application will fail once all attempts are exhausted.

### High-Availability on YARN  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#high-availability-on-yarn)

High-Availability on YARN is achieved through a combination of YARN and a [high availability service](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/ha/overview/).

Once a HA service is configured, it will persist JobManager metadata and perform leader elections.

YARN is taking care of restarting failed JobManagers. The maximum number of JobManager restarts is defined through two configuration parameters. First Flink’s [yarn.application-attempts](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#yarn-application-attempts) configuration will default 2. This value is limited by YARN’s [yarn.resourcemanager.am.max-attempts](https://hadoop.apache.org/docs/stable/hadoop-yarn/hadoop-yarn-common/yarn-default.xml), which also defaults to 2.
...
#### Container Shutdown Behaviour  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#container-shutdown-behaviour)
...
### Supported Hadoop versions.  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#supported-hadoop-versions)

Flink on YARN is compiled against Hadoop 2.10.2, and all Hadoop versions `>= 2.10.2` are supported, including Hadoop 3.x.

For providing Flink with the required Hadoop dependencies, we recommend setting the `HADOOP_CLASSPATH` environment variable already introduced in the [Getting Started / Preparation](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/#preparation) section.
...
### Running Flink on YARN behind Firewalls  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#running-flink-on-yarn-behind-firewalls)
...
The configuration parameter for specifying the REST endpoint port is [rest.bind-port](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#rest-bind-port). This configuration option accepts single ports (for example: “50010”), ranges (“50000-50025”), or a combination of both.

### User jars & Classpath  [\#](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/yarn/\#user-jars--classpath)
...
By default Flink will include the user-jars into the system classpath. This behavior can be controlled with the [yarn.classpath.include-user-jar](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#yarn-classpath-include-user-jar) parameter.
...
Please refer to the [Debugging Classloading Docs](https://nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/debugging/debugging_classloading/#overview-of-classloading-in-flink) for details.
# Task Lifecycle  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/\#task-lifecycle)

A task in Flink is the basic unit of execution. It is the place where each parallel instance of an
operator is executed. As an example, an operator with a parallelism of _5_ will have each of its
...
## Operator Lifecycle in a nutshell  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/\#operator-lifecycle-in-a-nutshell)
...
Given that an operator can have a user-defined function ( _UDF_), below each of the operator methods we also present
...
> state of the operator during its initial execution ( _e.g._ register any keyed state), and also the logic to retrieve its
...
where the UDF’s logic is invoked, _e.g._ the `map()` method of your `MapFunction`.

Finally, in the case of a normal, fault-free termination of the operator ( _e.g._ if the stream is
finite and its end is reached), the `finish()` method is called to perform any final bookkeeping
action required by the operator’s logic ( _e.g._ flush any buffered data, or emit data to mark end of
processing), and the `close()` is called after that to free any resources held by the operator
( _e.g._ open network connections, io streams, or native memory held by the operator’s data).
...
above whenever a checkpoint barrier is received. Checkpoints are performed during the processing phase, _i.e._ after the
...
to the specified [state backend](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/) from where it will be retrieved when
...
[Data Streaming Fault Tolerance](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/learn-flink/fault_tolerance/).

## Task Lifecycle  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/\#task-lifecycle-1)
...
describing the phases during a regular, fault-free execution of a task (see [Normal Execution](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/#normal-execution)), and
(a shorter) one describing the different sequence followed in case the task is cancelled (see [Interrupted Execution](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/#interrupted-execution)),
either manually, or due some other reason, _e.g._ an exception thrown during execution.

### Normal Execution  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/\#normal-execution)
...
2. when resuming from a [savepoint](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/savepoints/).
...
consuming the output of its predecessor. In this case, the `open()` method is called from the last operator, _i.e._ the
...
In the case of running till completion, _i.e._ there is no more input data to process, after exiting from the `run()`
method, the task enters its shutdown process. Initially, the timer service stops registering any new timers ( _e.g._ from
...
[wait for the final checkpoint completed](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/datastream/fault-tolerance/checkpointing/#waiting-for-the-final-checkpoint-before-task-exit)
...
performs its task-specific cleanup, _e.g._ cleans all its internal buffers, and then performs its generic task clean up
...
### Interrupted Execution  [\#](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/internals/task_lifecycle/\#interrupted-execution)

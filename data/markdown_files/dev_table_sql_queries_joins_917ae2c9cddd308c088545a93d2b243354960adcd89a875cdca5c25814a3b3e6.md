# Joins  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#joins)
...
## Regular Joins  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#regular-joins)
...
### INNER Equi-JOIN  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#inner-equi-join)
...
### OUTER Equi-JOIN  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#outer-equi-join)
...
### Multiple Regular Joins  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#multiple-regular-joins)

If you are facing issues with the performance of your multiple chained joins and these generate a lot of state, consider using the `MultiJoin` operator. See [tuning multiple regular joins](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/tuning/#multiple-regular-joins) for details.

## Interval Joins  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#interval-joins)

Returns a simple Cartesian product restricted by the join condition and a time constraint. An interval join requires at least one equi-join predicate and a join condition that bounds the time on both sides. Two appropriate range predicates can define such a condition (<, <=, >=, >), a BETWEEN predicate, or a single equality predicate that compares [time attributes](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/time_attributes/) of the same type (i.e., processing time or event time) of both input tables.
...
## Temporal Joins  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#temporal-joins)

A Temporal table is a table that evolves over time - otherwise known in Flink as a [dynamic table](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/dynamic_tables/). Rows in a temporal table are associated with one or more temporal periods and all Flink tables are temporal(dynamic).
...
### Event Time Temporal Join  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#event-time-temporal-join)

Event Time temporal joins allow joining against a [versioned table](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/versioned_tables/).
...
In contrast to [regular joins](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/#regular-joins), the previous temporal table results will not be affected despite the changes on the build side.
Compared to [interval joins](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/#interval-joins), temporal table joins do not define a time window within which the records will be joined.
...
### Processing Time Temporal Join  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#processing-time-temporal-join)
...
In contrast to [regular joins](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/#regular-joins), the previous temporal table results will not be affected despite the changes on the build side.
Compared to [interval joins](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/#interval-joins), temporal table joins do not define a time window within which the records join, i.e., old rows are not stored in state.

### Temporal Table Function Join  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#temporal-table-function-join)

The syntax to join a table with a [temporal table function](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/temporal_table_function/) is the same as in Join with [Table Function](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/#table-function).
...
## Lookup Join  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#lookup-join)
...
The lookup join uses the above [Processing Time Temporal Join](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/#processing-time-temporal-join) syntax with the right table to be backed by a lookup source connector.
...
## Array, Multiset and Map Expansion  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#array-multiset-and-map-expansion)
...
## Table Function  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#table-function)

Joins a table with the results of a table function. Each row of the left (outer) table is joined with all rows produced by the corresponding call of the table function. [User-defined table functions](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/functions/udfs/#table-functions) must be registered before use.

### INNER JOIN  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#inner-join)
...
### LEFT OUTER JOIN  [\#](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/sql/queries/joins/\#left-outer-join)
